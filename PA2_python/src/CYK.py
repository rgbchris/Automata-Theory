...
def calcCYK(w):
    ...
    # bottom row
    for i, sym_indx in enumerate(w):
      # i = Position Indices [0, 1, 2, 3, 4]
      # j = Symbol Indices [0, 1, 0, 1, 0]
    
      for j in range(VarNum):
        X[i][i][j] = existProd(j, sym_indx, -1)
    ...
    '''Step 1 is to fill the diagonals; ie the X(i,i) for each unit productions'''
